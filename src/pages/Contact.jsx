...
const initialForm = { name: '', email: '', category: contactCategories[0], subject: '', message: '' };
...
export default function Contact() {
  ...
  return (
    <>
      <Seo title="Contact" description="Get in touch with the CampusLYT team for support, feedback, partnerships or reports." path="/contact" />
      <header className="page-hero">
        <div className="container">
          <span className="eyebrow">Support</span>
          <h1>We're here to help</h1>
          <p>Questions, feedback, or something not working right? Send us a message.</p>
        </div>
      </header>
          ...
          {status === 'success' ? (
            ...
            <form className="contact__form" onSubmit={handleSubmit}>
              <div className="contact__row">
                ...
                <label>
                  Email
                  <input required type="email" value={form.email} onChange={update('email')} placeholder="you@campus.edu" />
                </label>
              </div>
              ...
              <label>
                Subject
                <input required value={form.subject} onChange={update('subject')} placeholder="How can we help?" />
              </label>
              ...
              {status === 'error' && (
                <div className="contact__error"><AlertCircle size={16} /> {errorMsg}</div>
              )}
